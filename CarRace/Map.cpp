...
void Map::initBackgroundTexture()
{
    for (int i = 0; i < 3; ++i)
    {
        backgroundTexture[i].loadFromFile("images\\background\\background" + std::to_string(i) + ".png");

        backgroundTexture[i].setRepeated(true);

        if (!backgroundTexture[i].loadFromFile("images\\background\\background" + std::to_string(i) + ".png")) 
        {
...
void Map::initFont()
{
    ...
    font2.loadFromFile("font\\font.ttf");
    start.setPosition(width / 2 - 315, height / 2 - 27);
    start.setFont(font2);
...
void Map::initObstacles()
{
    for (int i = 1; i < 20; i++)
    {
        t[i].loadFromFile("images\\obstacles\\" + to_string(i) + ".png");
        t[i].setSmooth(true);
        obstacle[i].setTexture(t[i]);
    }

    lifes.loadFromFile("images\\obstacles\\heart.png");
    lifes.setSmooth(true);
    life.setTexture(lifes);
